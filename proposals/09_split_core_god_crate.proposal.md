...
1.  **Create New Focused Crates:** For each identified domain (`auth`, `config`, `workspaces`), a new, dedicated crate will be created (e.g., `crates/codebuddy-auth`).
...
### 09a: Extract `codebuddy-auth`
- [ ] Create the new crate `crates/codebuddy-auth`.
- [ ] Move the `auth` module from `codebuddy-core/src/` to `codebuddy-auth/src/`.
- [ ] Update `codebuddy-auth/Cargo.toml` with dependencies that were previously required by `codebuddy-core` for the `auth` module.
...
### 09b: Extract `codebuddy-config`
- [ ] Create the new crate `crates/codebuddy-config`.
- [ ] Move the `config` and `refactor_config` modules from `codebuddy-core/src/` to the new crate.
- [ ] Update `codebuddy-config/Cargo.toml` with its required dependencies.
...
1.  New crates `codebuddy-auth`, `codebuddy-config`, and `codebuddy-workspaces` exist and are used by the workspace.
