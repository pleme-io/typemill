# Proposal: Analysis Subsystem Architecture

**Status**: Design Proposal - Ready for Implementation
**Date**: 2025-10-09
**Related**: 92_ADVANCED_ANALYSIS_VISION.md
...
## 2. Clean Separation Points

### 2.1 Current Architecture Analysis

**What analysis code needs:**
```rust
// From analysis_handler.rs:144
async fn analyze_dead_code(
    lsp_adapter: Arc<DirectLspAdapter>,  // ← LSP communication
    workspace_path: &str,                 // ← File discovery
    config: AnalysisConfig,               // ← Configuration
) -> ServerResult<Vec<DeadSymbol>>
```

**Key dependencies identified:**
1. **LSP Communication** → `DirectLspAdapter` for `workspace/symbol`, `textDocument/references`
2. **File Discovery** → Walking workspace to find source files
3. **Configuration** → Analysis parameters (symbol kinds, concurrency, timeouts)
4. **Result Types** → `DeadSymbol`, `AnalysisConfig`

**What can be extracted:**
- Core analysis algorithms (reference counting, graph building)
- Configuration parsing and validation
- Result type definitions
- File discovery utilities

**What must stay in handlers:**
- LSP adapter integration (bridges to `DirectLspAdapter`)
- MCP tool registration
- Tool call parameter parsing
- Response formatting
...
### 4.1 Fast Unit Tests (Analysis Crates)
...
### 4.2 Integration Tests (Handler Layer)
...
### Phase 1: Create Common Infrastructure (1 day)
...
### Phase 2: Extract Dead Code Analysis (1 day)
...
### Phase 3: Add Feature Flag Integration (2 hours)
...
### Phase 4: Add Deep Dead Code Analysis (NEW FEATURE)
...
1. ✅ **Review this proposal** → Get feedback on API design
2. ⬜ **Implement Phase 1** → Create `cb-analysis-common` crate
3. ⬜ **Implement Phase 2** → Extract `find_dead_code` to `cb-analysis-dead-code`
4. ⬜ **Validate iteration speed** → Measure test cycle time improvement
5. ⬜ **Add deep dead code analysis** → Prove the concept works for new features
