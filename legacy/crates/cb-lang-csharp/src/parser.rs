...
use cb_plugin_api::{
    ParsedSource, PluginError, PluginResult, SourceLocation, Symbol, SymbolKind,
};
use regex::Regex;
    ...
    fn from(json_symbol: JsonSymbol) -> Self {
        ...
        Symbol {
            ...
            location: SourceLocation {
                line: json_symbol.location.start_line,
                column: json_symbol.location.start_column,
            },
            documentation: None,
        }
    }
...
fn parse_with_ast(source: &str) -> PluginResult<Vec<Symbol>> {
    let parser_path = Path::new("crates/cb-lang-csharp/csharp-parser");

    if !parser_path.exists() {
        return Err(PluginError::internal(
            "C# parser executable not found at 'crates/cb-lang-csharp/csharp-parser'. Please run 'make build-parsers' in the project root.",
        ));
    }
...
"#;
    ...
    fn test_parse_with_ast_success() {
        ...
        if !Path::new("crates/cb-lang-csharp/csharp-parser").exists() {
