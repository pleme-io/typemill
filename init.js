...
`);
    ...
    console.log(`   Found ${extensions.size} file types: ${Array.from(extensions).slice(0, 10).join(", ")}${extensions.size > 10 ? "..." : ""}
`);
  }
  const relevantServers = [];
  const irrelevantServers = [];
...
function generateConfigWithComments(relevantServers, irrelevantServers) {
  const lines = [];
  lines.push("{");
  ...
  lines.push("  //");
  lines.push("  // This file configures Language Server Protocol (LSP) servers for different file types.");
  ...
  lines.push("  //   - restartInterval: Auto-restart interval in minutes (optional, disabled by default)");
...
function generateServerConfig(server, commented, isLast) {
  const lines = [];
  ...
  lines.push(`${prefix}  "command": ${JSON.stringify(server.command)}${server.rootDir || server.initializationOptions ? "," : ""}`);
  if (server.rootDir) {
    lines.push(`${prefix}  "rootDir": "${server.rootDir}"${server.initializationOptions ? "," : ""}`);
  }
  lines.push(`${prefix}  // "restartInterval": 30  // Uncomment to auto-restart every 30 minutes`);
  if (server.initializationOptions) {
    lines.push(`${prefix}  "initializationOptions": ${JSON.stringify(server.initializationOptions, null, 2).split(`
