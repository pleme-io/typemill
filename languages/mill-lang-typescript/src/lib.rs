...
use mill_plugin_api::{
    FileDiscovery, LanguagePlugin, ManifestData, ParsedSource, PluginApiError, PluginResult,
    StandardFileDiscovery,
};
use std::path::Path;

#[derive(Default)]
pub struct TypeScriptFileDiscovery;
...
#[async_trait]
impl FileDiscovery for TypeScriptFileDiscovery {
    async fn find_source_files(&self, root_path: &Path) -> PluginResult<Vec<std::path::PathBuf>> {
        let discovery = StandardFileDiscovery::new(TypeScriptPlugin::METADATA.extensions);
...
impl LanguagePlugin for TypeScriptPlugin {
    ...
    async fn parse(&self, source: &str) -> PluginResult<ParsedSource> {
        let symbols = parser::extract_symbols_async(source).await?;
    ...
    async fn list_functions(&self, source: &str) -> PluginResult<Vec<String>> {
        parser::list_functions_async(source).await
    }
