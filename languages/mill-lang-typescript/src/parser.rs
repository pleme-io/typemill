    ...
    Ok(ts_imports
        ...
        .map(|imp| ImportInfo {
            ...
            named_imports: imp
                .named_imports
                .iter()
                .map(|n| mill_foundation::protocol::NamedImport {
                    name: n.name.clone(),
                    alias: n.alias.clone(),
...
pub(crate) async fn extract_symbols_async(source: &str) -> PluginResult<Vec<Symbol>> {
    match extract_symbols_ast_async(source).await {
...
pub(crate) async fn list_functions_async(source: &str) -> PluginResult<Vec<String>> {
    let symbols = extract_symbols_async(source).await?;
    Ok(symbols
        .into_iter()
...
async fn extract_symbols_ast_async(source: &str) -> Result<Vec<Symbol>, PluginApiError> {
    const AST_TOOL_JS: &str = include_str!("../resources/ast_tool.js");
    let tool = SubprocessAstTool::new("node")
        .with_embedded_str(AST_TOOL_JS)
        .with_temp_filename("ast_tool.js")
...
"#;
        let result = extract_symbols_async(source).await;
