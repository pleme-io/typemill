...
pub async fn validate_no_circular_dependencies(
    ...
    project_root: &Path,
) -> PluginResult<CircularDependencyAnalysis> {
    info!(
    ...
    let source_crate_name = get_crate_name(source_crate_path).await?;
    let target_crate_name = get_crate_name(target_crate_path).await?;
    ...
    let dep_graph = build_workspace_dependency_graph(project_root).await?;
...
async fn build_workspace_dependency_graph(project_root: &Path) -> PluginResult<DependencyGraph> {
    debug!(workspace_root = %project_root.display(), "Building workspace dependency graph");
...
) -> PluginResult<Vec<ProblematicModule>> {
    debug!(
        source_crate = %source_crate_name,
        chain = ?dependency_chain,
