...
"#;
    ...
    fn test_import_parser_with_attributes() {
        ...
        assert!(!imports.is_empty()); // Should at least find Module2
    ...
    fn test_import_advanced_analyze_dependencies() {
        ...
        assert!(!graph.imports.is_empty(), "Should find at least one import");
    ...
    async fn test_plan_extract_constant_invalid_name() {
        ...
        assert!(
            result.is_err(),
            "Should reject non-SCREAMING_SNAKE_CASE name"
        );
    ...
    async fn test_extract_constant_negative_number() {
        ...
        let source = "let x = -42\n";
        let result = provider
            .plan_extract_constant(source, 0, 9, "NEGATIVE_VALUE", "test.swift")
            .await;
        assert!(
            result.is_ok(),
    ...
    async fn test_extract_constant_hex_literal() {
        ...
        let result = provider
            .plan_extract_constant(source, 0, 15, "MAGENTA_COLOR", "test.swift")
            .await;
    ...
    async fn test_extract_constant_binary_literal() {
        ...
        let result = provider
            .plan_extract_constant(source, 0, 14, "FLAG_MASK", "test.swift")
            .await;
        assert!(
            result.is_ok(),
            "Should extract binary literal: {:?}",
            result
        );
        let plan = result.unwrap();
    ...
    async fn test_extract_constant_octal_literal() {
        ...
        let result = provider
            .plan_extract_constant(source, 0, 20, "FILE_PERMISSIONS", "test.swift")
            .await;
    ...
    async fn test_extract_constant_scientific_notation() {
        ...
        let result = provider
            .plan_extract_constant(source, 0, 13, "EPSILON", "test.swift")
            .await;
        assert!(
            result.is_ok(),
        ...
        let plan = result.unwrap();
        assert!(
            plan.edits[0].new_text.contains("1e-5") || plan.edits[0].new_text.contains("EPSILON")
        );
    }
...
"#;
        let result = provider
            .plan_extract_constant(source, 0, 15, "GREETING", "test.swift")
            .await;
        assert!(
            result.is_ok(),
...
"#;
        let result = provider
            .plan_extract_constant(source, 0, 11, "TAX_RATE", "test.swift")
            .await;
        assert!(result.is_ok());
        let plan = result.unwrap();
        ...
        assert_eq!(
            plan.edits.len(),
            3,
            "Should have 1 insert + 2 replace edits (skip string content)"
        );
    }
    ...
    async fn test_extract_constant_skip_block_comment() {
        ...
        let source = "let x = 42\n/* comment with 42 */\nlet y = 42\n";
        let result = provider
            .plan_extract_constant(source, 0, 8, "VALUE", "test.swift")
            .await;
        assert!(result.is_ok());
        let plan = result.unwrap();
    ...
    async fn test_extract_constant_inline_block_comment() {
        ...
        let result = provider
            .plan_extract_constant(source, 0, 8, "NUMBER", "test.swift")
            .await;
        assert!(result.is_ok());
        let plan = result.unwrap();
        ...
        assert_eq!(
            plan.edits.len(),
            3,
            "Should skip literal in inline block comment"
        );
    }
    ...
    async fn test_extract_constant_float_with_exponent() {
        ...
        let result = provider
            .plan_extract_constant(source, 0, 12, "BIG_VALUE", "test.swift")
            .await;
        assert!(
            result.is_ok(),
            "Should extract float with uppercase E exponent: {:?}",
            result
        );
    }
...
"#;
        let result = provider
            .plan_extract_constant(source, 1, 8, "VALUE", "test.swift")
            .await;
        assert!(result.is_ok());
        let plan = result.unwrap();
    ...
    fn test_integration_rename_module_update_imports() {
        ...
        let refs = scanner
            ...
            .expect("Should scan");
        assert!(!refs.is_empty()); // Should find at least the import
    }
