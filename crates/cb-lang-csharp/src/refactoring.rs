...
pub fn plan_extract_function(
    ...
    let tree = parser.parse(source, None).ok_or_else(|| RefactoringError::Parse("Failed to parse C# source".to_string()))?;
    ...
    let start_node = find_node_at_point(root, start_point)
        .ok_or_else(|| RefactoringError::Analysis("Could not find a node at the start of the selection.".to_string()))?;
    let end_node = find_node_at_point(root, end_point)
        .ok_or_else(|| RefactoringError::Analysis("Could not find a node at the end of the selection.".to_string()))?;
    ...
    let new_method_text = format!("\n\n{}private void {}()\n{}{{\n{}{}\n{}}}\n", indent, function_name, indent, method_indent, selected_text.trim(), indent);

    let insert_edit = TextEdit {
        file_path: None,
        edit_type: EditType::Insert,
        location: CommonCodeRange::new(enclosing_method.end_position().row as u32 + 1, 0, enclosing_method.end_position().row as u32 + 1, 0).into(),
    ...
    let replace_edit = TextEdit {
        ...
        location: CommonCodeRange::new(range.start_line, range.start_col, range.end_line, range.end_col).into(),
...
pub fn plan_extract_variable(
    ...
    let insert_edit = TextEdit {
        file_path: None,
        edit_type: EditType::Insert,
        location: CommonCodeRange::new(insertion_statement.start_position().row as u32, 0, insertion_statement.start_position().row as u32, 0).into(),
        original_text: String::new(),
...
pub fn plan_inline_variable(
    ...
    let query = Query::new(tree_sitter_c_sharp::language(), &query_str).map_err(|e| RefactoringError::Query(e.to_string()))?;
...
class Program
{
    static void Main(string[] args)
    {
        ...
        let range = CodeRange { start_line: 6, start_col: 8, end_line: 6, end_col: 42 };
