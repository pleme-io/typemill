...
// Convert plugin errors to refactoring errors
...
def test():
    ...
    return x
"#;
        let plan = plan_extract_variable(source, 2, 8, 2, 22, Some("count".to_string()), "test.py").unwrap();
...
y = x + 5"#;
        ...
        assert_eq!(plan.edits.len(), 2); // 1 replacement + 1 deletion
