    ...
    for (line_num, line) in source.lines().enumerate() {
        if let Some(captures) = func_re.captures(line) {
            ...
            functions.push(PythonFunction {
                name: name.to_string(),
                start_line: line_num as u32,
                end_line: line_num as u32 + 10, // Estimated, refined by find_python_function_end
                args,
...
"#;
        ...
        let has_constant = symbols.iter().any(|s| s.name == "CONSTANT" && s.kind == SymbolKind::Constant);
        let has_function = symbols.iter().any(|s| s.name == "my_function" && s.kind == SymbolKind::Function);
        let has_class = symbols.iter().any(|s| s.name == "MyClass" && s.kind == SymbolKind::Class);
        let has_variable = symbols.iter().any(|s| s.name == "variable" && s.kind == SymbolKind::Variable);
