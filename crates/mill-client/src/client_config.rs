...
impl ClientConfig {
    ...
    pub fn suggest_actions(&self) -> Vec<String> {
        ...
        if self.token.is_none() {
            actions.push(
                "Set authentication token with --token or TYPEMILL_TOKEN environment variable"
                    .to_string(),
            );
        }
...
impl ConfigBuilder {
    ...
    pub fn with_env_overrides(mut self) -> Self {
        ...
        if let Ok(url) = std::env::var("TYPEMILL_URL") {
            debug!("Using URL from environment variable");
            self.config.url = Some(url);
        }

        if let Ok(token) = std::env::var("TYPEMILL_TOKEN") {
            debug!("Using token from environment variable");
            self.config.token = Some(token);
        }

        if let Ok(timeout) = std::env::var("TYPEMILL_TIMEOUT") {
...
mod tests {
    ...
    async fn test_config_builder_env_overrides() {
        ...
        let original_url = env::var("TYPEMILL_URL").ok();
        let original_token = env::var("TYPEMILL_TOKEN").ok();
        let original_timeout = env::var("TYPEMILL_TIMEOUT").ok();

        // Set test environment variables
        env::set_var("TYPEMILL_URL", "ws://env:4000");
        env::set_var("TYPEMILL_TOKEN", "env-token");
        env::set_var("TYPEMILL_TIMEOUT", "25000");
        ...
        // Clean up
        env::remove_var("TYPEMILL_URL");
        env::remove_var("TYPEMILL_TOKEN");
        env::remove_var("TYPEMILL_TIMEOUT");

        // Restore original values if they existed
        if let Some(url) = original_url {
            env::set_var("TYPEMILL_URL", url);
        }
        if let Some(token) = original_token {
            env::set_var("TYPEMILL_TOKEN", token);
        }
        if let Some(timeout) = original_timeout {
            env::set_var("TYPEMILL_TIMEOUT", timeout);
        }
    }
    ...
    async fn test_config_precedence_cli_over_env() {
        ...
        env::set_var("TYPEMILL_URL", "ws://env:5000");
        env::set_var("TYPEMILL_TOKEN", "env-token");
    ...
    async fn test_config_precedence_env_over_file() {
        ...
        env::set_var("TYPEMILL_URL", "ws://env:8000");
    ...
    async fn test_invalid_env_timeout_handled_gracefully() {
        env::set_var("TYPEMILL_TIMEOUT", "invalid-number");
