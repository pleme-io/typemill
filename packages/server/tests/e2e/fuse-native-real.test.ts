...
import { access, mkdir, readdir, readFile, rmdir, stat, writeFile, unlink } from 'node:fs/promises';
    ...
    try {
      switch (method) {
        ...
        case 'fuse/stat': {
            ...
            return { ...stats, mode: stats.mode, size: stats.size, mtime: stats.mtime, atime: stats.atime, ctime: stats.ctime, uid: stats.uid, gid: stats.gid };
        }
        case 'fuse/read': {
            if (!existsSync(fullPath)) throw new Error('File not found');
...
testSuite('Real FUSE Native Integration Tests', () => {
  ...
  test('should mount, read, write, and delete files through a real FUSE mount', async () => {
    ...
    const enhancedSession: EnhancedClientSession = { ...mockSession, projectRoot: workspace.workspaceDir, socket: {} as any, initialized: true, globalProjectId: workspace.globalProjectId, workspaceId: workspace.workspaceId, fuseMount: workspace.fuseMount, workspaceDir: workspace.workspaceDir, permissions: ['file:read', 'file:write'] } as any;
    const fuseMount = new FuseMount(enhancedSession, transport as any, workspace.fuseMount, { debugFuse: false, allowOther: false, defaultPermissions: true });
