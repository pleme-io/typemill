...
describe.skip('Batch Directory Move Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    writeFileSync(
      filePaths.main,
      `import { A } from './src/dir_a/file_a';
...
console.log(A, B);`
  ...
  it('should atomically fail and roll back all directory moves if one operation fails', async () => {
      ...
      operations: [
        {
          ...
          args: { old_path: filePaths.dirA, new_path: filePaths.destDirA },
          id: 'move-a',
        },
        {
          tool: 'rename_directory',
          args: { old_path: filePaths.dirB, new_path: filePaths.destDirB },
          id: 'move-b',
        },
        {
          tool: 'rename_directory',
          args: { old_path: filePaths.dirC, new_path: filePaths.destDirC },
          id: 'move-c-fail',
        }, // This will fail
      ],
    ...
    expect(response).toContain(
      'Rolling back atomic transaction - all operations have been reverted'
    );
    ...
    expect(readFileSync(filePaths.dirA_file, 'utf-8')).toBe(
      initialFileStates.get(filePaths.dirA_file)
    );
    expect(readFileSync(filePaths.dirB_file, 'utf-8')).toBe(
      initialFileStates.get(filePaths.dirB_file)
    );
