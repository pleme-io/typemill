...
pub enum Commands {
    ...
    /// Generate JWT authentication token (requires server config access)
    GenerateToken {
        /// Project ID to embed in token
        #[arg(long)]
...
async fn handle_generate_token(
    ...
    let auth_config = match config.server.auth {
        Some(a) => a,
        None => {
            eprintln!("❌ Authentication is not configured in .typemill/config.json or mill.toml");
            process::exit(1);
        }
    };
