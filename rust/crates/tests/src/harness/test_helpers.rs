...
impl BatchOperationHelper {
    ...
    pub async fn create_multiple_files(&self, files: &[(&str, &str)]) -> Result<Vec<Value>, Box<dyn std::error::Error>> {
        ...
        for (file_path, content) in files {
            let response = self.client.call_tool("create_file", json!({
                "file_path": file_path,
                "content": content
            })).await?;
    ...
    pub async fn read_multiple_files(&self, file_paths: &[&str]) -> Result<Vec<Value>, Box<dyn std::error::Error>> {
        ...
        for file_path in file_paths {
            let response = self.client.call_tool("read_file", json!({
                "file_path": file_path
            })).await?;
...
impl LspTestHelper {
    ...
    pub async fn verify_symbol_exists(&self, file_path: &str, symbol_name: &str) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        let exists = symbols.iter().any(|symbol| {
            symbol["name"].as_str().unwrap_or("").contains(symbol_name)
        });
...
impl ErrorTestHelper {
    ...
    pub async fn test_malformed_parameters(&self) -> bool {
        let malformed_calls = vec![
            ...
            ("find_definition", json!({"file_path": "test.ts", "line": "not_a_number", "character": 0})), // Invalid type
        ];
...
impl ResultVerifier {
    ...
    pub fn verify_performance_threshold(duration: Duration, threshold: Duration, operation: &str) -> bool {
        if duration > threshold {
            eprintln!("Performance warning: {} took {:?}, expected < {:?}", operation, duration, threshold);
            false
        } else {
            true
        }
