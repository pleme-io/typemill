...
async fn test_rename_file_plan_and_apply() {
    run_tool_test_with_plan_validation(
        ...
        |plan| {
            ...
            assert!(plan.get("fileChecksums").is_some(), "Plan should have fileChecksums");
            Ok(())
        },
        |ws| {
            assert!(!ws.file_exists("original.rs"), "Original file should be deleted");
            assert!(ws.file_exists("renamed.rs"), "New file should exist");
            assert_eq!(ws.read_file("renamed.rs"), "pub fn hello() {}\n", "Content should be preserved");
            Ok(())
        }
    ).await.unwrap();
}
...
async fn test_rename_file_dry_run_preview() {
    run_dry_run_test(
        ...
        |ws| {
            ...
            assert!(!ws.file_exists("test_renamed.rs"), "New file should NOT exist after dry run");
            Ok(())
        }
    ).await.unwrap();
}
