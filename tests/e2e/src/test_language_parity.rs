...
async fn run_language_parity_check(
    ...
    match inspect_res {
        ...
        Err(e) => {
             ...
             println!("{} inspect_code returned error (expected in some envs): {}", lang_name, err_msg);
             ...
             assert!(!err_msg.contains("Method not found"), "inspect_code tool missing/not registered for {}", lang_name);
        }
    }
    ...
    let search_res = client.call_tool("search_code", json!({ "query": query_symbol })).await;
    match search_res {
         Ok(_) => {
         ...
         Err(e) => {
             let err_msg = e.to_string();
             println!("{} search_code returned error (expected in some envs): {}", lang_name, err_msg);
             assert!(!err_msg.contains("Method not found"), "search_code tool missing/not registered for {}", lang_name);
         }
    }
    ...
    match rename_res {
        ...
        Err(e) => {
            let err_msg = e.to_string();
            println!("{} rename_all returned error (expected in some envs): {}", lang_name, err_msg);
            assert!(!err_msg.contains("Method not found"), "rename_all tool missing/not registered for {}", lang_name);
        }
    }
}
...
async fn test_rust_parity() {
    let workspace = TestWorkspace::new();
    workspace.create_file("Cargo.toml", r#"[package]
name = "test_project"
version = "0.1.0"
edition = "2021"
