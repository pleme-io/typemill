...
fn setup_rust_workspace() -> TestWorkspace {
    ...
    let status = Command::new("git")
        .args(["clone", "--depth", "1", "https://github.com/dtolnay/itoa.git", "."])
...
async fn test_user_scenario() {
    ...
    client.wait_for_lsp_ready(&lib_rs, 60000).await.expect("LSP failed to index");

    // 1. Inspect
    println!("Testing inspect_code...");
    let inspect_res = client.call_tool("inspect_code", json!({
        "filePath": lib_rs.to_string_lossy(),
        "line": 0,
        "character": 0,
        "include": ["definition", "typeInfo"]
    })).await.expect("inspect_code failed");
    println!("Inspect Result: {:?}", inspect_res);

    // 2. Search
    println!("Testing search_code...");
    let search_res = client.call_tool("search_code", json!({
        "query": "Buffer"
    })).await.expect("search_code failed");
     // Check results
    let results = search_res.get("result").and_then(|r| r.get("results")).and_then(|r| r.as_array());

    if let Some(res) = results {
        if res.is_empty() {
            println!("Warning: search_code returned empty results for 'Buffer'. Indexing might be slow.");
        } else {
             println!("Found {} results for 'Buffer'", res.len());
        }
    } else {
        println!("Warning: search_code result format unexpected: {:?}", search_res);
    }
    ...
    match rename_res {
        Ok(res) => {
            let rename_status = res.get("result")
            ...
            assert!(
                ...
                "Rename status should be preview or success, got {:?}", rename_status
            );
            println!("Rename Result: {:?}", res);
        }
        Err(e) => {
            println!("Rename failed: {}. This might happen if LSP hasn't indexed the new file yet.", e);
    ...
    let relocate_res = client.call_tool("relocate", json!({
        "target": {
            "kind": "file",
        ...
        "options": { "dryRun": true }
    })).await.expect("relocate failed");

    let relocate_status = relocate_res.get("result")
        .and_then(|r| r.get("content"))
        .and_then(|c| c.get("status"))
        .and_then(|s| s.as_str());
    ...
    assert!(
        ...
        "Relocate status should be preview or success, got {:?}", relocate_status
    );
    ...
    let prune_res = client.call_tool("prune", json!({
        "target": {
            "kind": "file",
            "filePath": delete_path.to_string_lossy()
        },
        "options": { "dryRun": false }
    })).await.expect("prune failed");

    let prune_status = prune_res.get("result")
        .and_then(|r| r.get("content"))
        .and_then(|c| c.get("status"))
        .and_then(|s| s.as_str());
    ...
    let refactor_res = client.call_tool("refactor", json!({
        "action": "extract",
        "params": {
            "kind": "variable",
    ...
    match refactor_res {
        ...
        Err(e) => println!("Refactor failed (expected if LSP capability missing): {}", e),
    }

    // 7. Workspace
    println!("Testing workspace...");
    let workspace_res = client.call_tool("workspace", json!({
        "action": "verify_project",
        "params": {}
    })).await.expect("workspace verify failed");
